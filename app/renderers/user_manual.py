...
def _sanitize_module(m: dict, mod_spec: dict, template: str) -> dict:
    ...
    for k in ("filters", "operations", "columns", "rows", "steps", "fields", "notes",
              "business_rules", "errors", "screenshot_captions"):
...
async def render(
    ...
    for i, mod_data in enumerate(ui["modules"][:len(spec["functions"])]):
        # N8: 优先用 LLM 给的业务化 caption，按 (item 在该 module 内的顺序) 取
        ...
        for item, png in zip(shot_plan, pngs):
            if item["module_idx"] == i:
                if local_idx < len(llm_captions) and isinstance(llm_captions[local_idx], str) and llm_captions[local_idx].strip():
                    cap = llm_captions[local_idx].strip()
                else:
                    cap = f"{mod_data.get('name') or spec['functions'][i]['name']} - {_SUBPAGE_CAPTION.get(item['kind'], item['kind'])}"
                shots_for_mod.append({"img": png, "caption": cap})
                local_idx += 1
